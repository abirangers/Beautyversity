@foreach($articles as $article)
    <div class="bg-white rounded-lg shadow-sm hover:shadow-xl transition-shadow duration-300 overflow-hidden">
        ...
        <div class="p-6">
            ...
            <h2 class="text-xl font-bold text-gray-800 mb-3 line-clamp-2">
                <a href="{{ route('article.show', $article->id) }}" class="hover:text-blue-600 transition-colors">
                    {{ $article->title }}
                </a>
            </h2>
            @if ($article->excerpt)
            ...
            @endif
            <a href="{{ route('article.show', $article->id) }}"
                class="font-semibold text-primary-60 hover:underline">Baca Selengkapnya</a>
        </div>
    </div>
@endforeach